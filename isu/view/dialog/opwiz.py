...
def help_url(page):
    ...
    major_version = qVersion().split(b'.')[0]
...
class WidgetGallery(QDialog):
    ...
    def __init__(self, parent = None):
        ...
        help_shortcut.activated.connect(self.help_on_current_widget)  # type: ignore
        ...
        dialog_buttonbox = QDialogButtonBox(QDialogButtonBox.Help |
                                            QDialogButtonBox.Close)  # type: ignore
        init_widget(dialog_buttonbox, "dialogButtonBox")
        dialog_buttonbox.helpRequested.connect(launch_module_help)  # type: ignore
        dialog_buttonbox.rejected.connect(self.reject)  # type: ignore
    ...
    def setVisible(self, visible):
        ...
        if visible:
            self.windowHandle().screenChanged.connect(self.update_systeminfo)  # type: ignore
...
utils.show(__name__, WidgetGallery)